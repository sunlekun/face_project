...
<div class="tab-content">
  <dl>
    ...
    <dd>
      <div class="rule-single-select">
        <select name="role_type" id="role_type" disabled datatype="*" errormsg="请选择角色类型！" sucmsg=" ">
  ...
  <dl>
    <dt>角色名称</dt>
    <dd><input name="role_name" type="text" id="role_name" readonly value='${role.role_name}' class="input normal" sucmsg=" " /> <span class="Validform_checktip" id="msg">*角色中文名称，100字符内</span></dd> 
  </dl>   
  <dl>
    <dt>管理权限</dt>
    <dd>
      ...
      <div class="tree-list">
       ...
       <li class="layer-${nav.level }">
            <div class="tbody">
              ...
              <div class="col col-2">
                <span id="rptList_cblActionType_${status.index}" class="cbllist"> 
                 <c:forEach items="${nav.action_type.split(',') }" var="act" varStatus="actions" >
                     ...
                     <input id="rptList_cblActionType_${status.index}_${actions.index}_${status.index}" type="checkbox" disabled="disabled"   ${fn:contains(roleNavs,sum)?"checked='true'":""}    name="actList" value="${nav.id },${nav.name },${act }" /> 
              ...
              <div class="col col-3">
                <input name="checkAll" type="checkbox" disabled="disabled" />
              </div>
            </div>
       </li>
