...
function infoFormatter( value, row, index){ 
 ...
 <%--  return "<a class='update'  href = '<%=path%>/randomCheck/toRandomCheckEdit?id="+value+"'>修改</a><br>" ; --%>
    ...
    window.actionEvents = {
     ...
     document.getElementById("form1").action="<%=path%>/randomCheck/toRandomCheckList"; 
   ...
   function deleteDiaryList() {  
    ...
    if (rows.length== 0) {  
     ...
     if(con==true)
     {
      ...
      if(from!=null){
        from.action="<%=path%>/randomCheck/randomCheckDelete"; 
        from.submit();
       }  
...
<body class="mainbody">
<form method="post"  action="<%=path%>/randomCheck/toRandomCheckList"  id="form1">
